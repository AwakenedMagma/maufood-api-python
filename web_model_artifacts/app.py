...
def get_filtered_menu_ids(kategori='', bahan='', rasa=''):
    ...
    if kategori and str(kategori).strip():
        mask &= (df_menu['Kategori_Hidangan'].astype(str).str.strip().str.lower() == str(kategori).strip().lower())
        
    if bahan and str(bahan).strip():
        mask &= (df_menu['Bahan_Baku'].astype(str).str.strip().str.lower() == str(bahan).strip().lower())
        
    if rasa and str(rasa).strip():
        mask &= (df_menu['Rasa_Dominan'].astype(str).str.strip().str.lower() == str(rasa).strip().lower())
...
def recommend():
    try:
        ...
        bahan = data.get('bahan') or ''
        rasa = data.get('rasa') or ''
